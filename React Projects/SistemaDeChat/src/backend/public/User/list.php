...
// Verifica se o método da requisição é GET para listar usuários
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // Instancia o controlador para gerenciar os dados
    $controller = new UsernameController();

    // Chama o método listUsers do controlador
    $result = $controller->listUsers();

    if ($result !== false) {
        $response['status'] = 'success';
        $response['data'] = $result; // Adiciona os usuários na resposta
    } else {
...
// Verifica se o método da requisição é POST para criar usuários
